...
class MarketNode(Node):
    ...
    def __init__(self, name):
        ...
        # 使用参数设置发布者
        self.index_publisher = self.create_publisher(Float32, f'{MarketNode.TOPIC_PREFIX}/index', 10)
        self.spot_publisher = self.create_publisher(StockSpotDataArray, f'{MarketNode.TOPIC_PREFIX}/spot', 10)
...
def main(args=None):
    try:
        ...
        node = MarketNode(f"{MarketNode.TOPIC_PREFIX}_publisher")
        rclpy.spin(node)
        node.destroy_node()
        rclpy.shutdown()
